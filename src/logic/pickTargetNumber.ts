...
export function pickTargetNumber(cards: CardData[]): Challenge {
  ...
  const hardestAnswersFirst = Array.from(collectedSolutionsMap.values()).sort((a,b) => a.estimatedDifficulty - b.estimatedDifficulty).reverse()
  ...
  const hardestAnswers = hardestAnswersFirst.filter(a => a.estimatedDifficulty > hardestDifficulty - 1.5) // wriggle-room - want multiple candidate targets
  const pickedTarget = hardestAnswers[Math.floor(Math.random() * hardestAnswers.length)]
...
function buildNextSteps(arg1: CardData, arg2: CardData): [SolutionStep, number][] {
  ...
  if (arg1.value % arg2.value === 0){
    res.push([{
      ...
      arg1,
      arg2
    }, Math.floor(arg1.value / arg2.value)])
  }

  return res
}

function buildNextPotentialSolutions(arg1: CardData, arg2: CardData, others: CardData[], stepsSoFar: SolutionStep[]): PotentialSolution[]
{
...
function searchForSolutions(map: PotentialSolutionMap, cards: CardData[], solutionSteps: SolutionStep[]) {
  if (cards.length < 2) {
    return;
  }
  for (let i = 0; i < cards.length - 1; i++) {
    for (let j = i+1; j < cards.length; j++) {
      ...
      others.splice(j, 1);
      others.splice(i, 1);
      const nextSteps = buildNextPotentialSolutions(arg1,arg2, others,solutionSteps)
      for (const sol of nextSteps) {
        const newVal = sol.latestValue!
        if (100<=newVal && newVal <= 999)
        {
            const currentMapVal = map.get(newVal)
            if (!currentMapVal || currentMapVal.estimatedDifficulty > sol.estimatedDifficulty)
            {
              map.set(newVal,sol)
            }
        }
...
function estimateSolutionDifficulty(steps:SolutionStep[]): number {
  return steps.map(s => estimateOperationDifficulty(s)).reduce((s,v) => s+v, 0)
}
